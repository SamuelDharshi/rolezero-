...
module sui_roles::role {
    ...
    public struct PaymentDeadlineExtended has copy, drop {
        role_id: address,
        payment_index: u64,
        recipient: address,
        old_scheduled_time: u64,
        new_scheduled_time: u64,
    }
    ...
    // Extend individual payment deadline (owner only)
    public fun extend_payment_deadline(
        role: &mut Role,
        payment_index: u64,
        new_scheduled_time: u64,
        clock: &Clock,
        ...
        let payment = vector::borrow_mut(&mut role.payments, payment_index);
        
        // Can only extend if not yet executed
        assert!(!payment.executed, ERR_NOT_TIME);
        assert!(new_scheduled_time > payment.scheduled_time, ERR_NOT_TIME);
        assert!(new_scheduled_time <= role.expiry_time, ERR_NOT_TIME); // Must be within role expiry

        let old_scheduled_time = payment.scheduled_time;
        payment.scheduled_time = new_scheduled_time;

        event::emit(PaymentDeadlineExtended {
            role_id: object::uid_to_address(&role.id),
            payment_index,
            recipient: payment.recipient,
            old_scheduled_time,
