...
export const useAutoPaymentMonitor = (
  ...
  useEffect(() => {
    ...
    const checkAndExecute = async () => {
      ...
      const readyPayments = roleData.payments.filter(p => {
        const paymentKey = `${p.recipient.toLowerCase()}_${p.amount}_${p.scheduledTime}`;
        const isExecuted = p.executed || 
          roleData.executedPayments.some(
            ep => ep.recipient.toLowerCase() === p.recipient.toLowerCase() && 
                 Math.abs(ep.amount - p.amount) < 100000
          ) ||
          processedPaymentsRef.current.has(paymentKey);
      ...
      if (readyPayments.length > 0 && roleData.remainingBalance > 0) {
        ...
        // Mark payments as being processed
        readyPayments.forEach(p => {
          const paymentKey = `${p.recipient.toLowerCase()}_${p.amount}_${p.scheduledTime}`;
    ...
    // Check every 30 seconds for payments that are ready
  ...
  }, [account, isCreator, roleData, isActive, status.autoExecuteEnabled]);
