  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const fundingHistory = txBlocks.data
        .filter((tx: any) => tx.events?.some((e: any) => e.type.includes('::role::FundEvent')))
        .map((tx: any) => {
          const fundEvent = tx.events?.find((e: any) => e.type.includes('::role::FundEvent'));
      ...
      const executedPayments = txBlocks.data
        .filter((tx: any) => tx.events?.some((e: any) => e.type.includes('::role::PaymentExecuted')))
        .map((tx: any) => {
          const paymentEvent = tx.events?.find((e: any) => e.type.includes('::role::PaymentExecuted'));
    ...
    enabled: !!roleId,
    refetchInterval: 20000, // Increased from 5s to 20s to prevent rate limiting
